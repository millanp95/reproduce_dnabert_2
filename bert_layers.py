...
class BertUnpadSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor,
        ...
        if self.p_dropout or flash_attn_qkvpacked_func is None:
            ...
            k = qkv[:, :, 1, :, :].permute(0, 2, 1, 3)  # b h s d (fixed: removed transpose)
            v = qkv[:, :, 2, :, :].permute(0, 2, 1, 3)  # b h s d
            
            # Use PyTorch's scaled_dot_product_attention with ALiBi bias
            # bias shape: (batch, heads, seq_len, seq_len)
            # scaled_dot_product_attention expects attn_mask in this format
            if self.p_dropout > 0:
                # If dropout > 0, use manual implementation (scaled_dot_product_attention doesn't support dropout with bias)
                attention_scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.attention_head_size)
                ...
                attention = F.scaled_dot_product_attention(
                    q, k, v, 
                    attn_mask=bias,  # ALiBi bias as attention mask
                    dropout_p=0.0,   # No dropout in this path
                    is_causal=False
                )
